...
def ai_response(user_text):
    if "fever" in user_text.lower() or "cough" in user_text.lower():
        return "🩺 Health AI Tip: Monitor symptoms and consult a doctor if necessary."
    return "🩺 Health AI Tip: Maintain a healthy lifestyle."

for msg in st.session_state.messages:
    with st.chat_message(msg["role"]):
        st.markdown(msg["content"])

user_input = st.chat_input("Ask Health AI")
if user_input:
...
if st.button("Generate Report"):
    report = f"""
Ebiklean Global AI Health Checker Report
User: {st.session_state.user}
...
"""
    ...
    st.download_button(
        "Download Report",
        report,
        file_name="ai_health_checker_report.txt",
        mime="text/plain"
    )

st.caption("© 2026 Ebiklean Global • AI Health Checker")